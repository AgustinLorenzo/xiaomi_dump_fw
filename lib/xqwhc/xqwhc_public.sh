...
export USE_DUAL_BAND_BH=1
export USE_ONLY_5G_BH=2
export USE_ONLY_5G_IND_VAP_BH=3
export USE_DUAL_BAND_IND_VAP_BH=4
export BH_METHOD="$USE_ONLY_5G_IND_VAP_BH"
export XQWHC_DEBUG=0 # miwifi set to 1 for test
#export XQWHC_DEBUG_EVT=1    # in case wifi calib mac same, disable after DVT done
...
# print level
export XQWHC_PRT_ERR=1      # erro
export XQWHC_PRT_INFO=2     # erro + info
export XQWHC_PRT_DEBUG=3    # erro + info + debug
export XQWHC_PRT_LEVEL="$XQWHC_PRT_INFO"
[ "$XQWHC_DEBUG" = "1" ] && XQWHC_PRT_LEVEL="$XQWHC_PRT_DEBUG"

. /lib/functions.sh
. /usr/share/libubox/jshn.sh


LOG_FILE="/tmp/log/xqwhc.log"
TMP_LOG_SIZE=600000
...
########  xqwhc log rewind activate in xqwhc_superv
if false; then
    # if log size > 150K, save and recreate
    [ -f $file -a `stat -c%s $file` -gt $TMP_LOG_SIZE ] && {
        echo -e "${time}    new log file" > "$file"
    }
fi
...
WHC_LOGD()
{
    [ "$XQWHC_PRT_LEVEL" -lt "$XQWHC_PRT_DEBUG" ] && return 0

    local __msg="$1"
    [ "$XQWHC_DEBUG" = "1" ] && {
        echo "xqwhc_debug $__msg"
    }
    #logger -p 4 -t xqwhc_debug "$__msg" 2>/dev/null
    WHC_TRACE "xqwhc_debug $__msg"
}

WHC_LOGI()
{
    [ "$XQWHC_PRT_LEVEL" -lt "$XQWHC_PRT_INFO" ] && return 0

    local __msg=" $1"
    [ "$XQWHC_DEBUG" = "1" ] && {
        echo "xqwhc_info $__msg"
    }
    #logger -p 3 -t xqwhc_info "$__msg" 2>/dev/null
    WHC_TRACE "xqwhc_info $__msg"
}

WHC_LOGE()
{
    [ "$XQWHC_PRT_LEVEL" -lt "$XQWHC_PRT_ERR" ] && return 0

    local __msg="$1"
    [ "$XQWHC_DEBUG" = "1" ] && {
        echo "xqwhc_error $__msg"
    }
    #logger -p 2 -t xqwhc_error "$__msg" 2>/dev/null
    WHC_TRACE "xqwhc_error $__msg"
}
