...
# miwifi: all bridge ifaces include lan & wan
bridge_eth_ifaces=""

IS_GW_REACHABLE=0
GW_NOT_REACHABLE_TIMESTAMP=0

# miwifi: check gw reachable in case: eth backhaul but unreachable
gw_unreachable=0
MAX_UNREACHABLE=5
...
#    current_backhaul_24g_rate=`iwconfig $sta_iface_24g | grep "Rate:" | awk '{print $2}' | cut -d: -f 2 | awk '{print $1}'`
    ...
    # miwifi store ether_iface on hyfi-bridge
    ...
    for ether_iface in $ether_ifaces; do
        if [ "$ether_iface" = "$__gw_iface" ]; then
            ...
            # miwifi recheck and update gw reachable status
            if [ "$IS_GW_REACHABLE" -eq 0 ]; then
...
__gwmon_miwifi_eth_bh_exist()
{
        if [ "$router_detected" -gt 0 ]; then
            #__gwmon_debug " @@ gwmon_check router_detected=$router_detected ! "
        ...
        # miwifi: note1: in wlan bh situation, when eth bh plugin, do NOT mean gw is reachable cause of bridge learn?
        # so we decide once router_detect as eth bh find
...
__gwmon_check() {
    if [ "$router_detected" -eq 0 ]; then
        ...
        # check to find a eth backhaul from eth1 eth0
        __gwmon_miwifi_eth_bh_exist
        return $?
    else
