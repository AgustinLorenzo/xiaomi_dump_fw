...
__repacd_restart_hyd() {
    hyd_running=$(ps | grep hyd | grep -v grep)
    if [ -z "$hyd_running" ]; then
        __repacd_wifimon_debug "bSTA Associated; restarting the hyd"
...
__repacd_wifimon_measure_link() {
    ...
    if [ "$rssi" -gt -95 ] && [ "$rssi" -lt 0 ]; then
        if [ "$rssi_num" -lt "$rssi_samples" ]; then
            ...
            __repacd_wifimon_set_target_bssid "$current_bssid"
        fi
    fi
}
...
__repacd_wifimon_set_target_bssid() {
    ...
    wpa_cli -p "/var/run/wpa_supplicant-$sta_iface" -i "$sta_iface" \
        bssid 0 "$desired_bssid"
        ...
        if __repacd_wifimon_check_associated "$1" "$2" "$3" 0 "$4" "$5" "$6"; then
            ...
            if [ "$wps_in_progress" -gt 0 ]; then
                ...
                if __repacd_wifimon_is_timeout $assoc_start_time $assoc_timeout; then
                    ...
                    __repacd_wifimon_set_target_bssid "$WIFIMON_WILDCARD_BSSID"
                fi
            fi
        fi
    fi
