...
repacd_backhaulmgrmon_check() {
  ...
  if [ "$eth_backhaul_mono" -gt 0 -a "$miwifi_ethbh_mono_en" -gt 0 ]; then
        ...
        #if [ ! "$is_plc_iface_brought_down" -eq 1 ]; then
        #    __repacd_backhaulmgrmon_info " miwifi eth bh mono, shutdown plc bh $plc_iface "
        ...
        #if [ "$is_plc_iface_brought_down" -eq 1 ];then
        #    __repacd_backhaulmgrmon_info " miwifi bring up plc bh"
