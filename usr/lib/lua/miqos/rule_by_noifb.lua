...
--    local guestexpr = string.format("%s del dev br-guest root ", const_tc_qdisc)
--    table.insert(tblist,guestexpr)
...
local function change_single_node_limit(node_name, devs, bands)
    ...
    for _dir,v in pairs(devs) do
        ...
        local class_id, parent_id = class.id, CLASS_HIER['root'].id
        local ratelimit=bands[_dir]
        local lrate=math.ceil(class.rate*ratelimit)
...
local function check_if_need_update_service_mainframe(clean_flag, origin_disc, devs, bands)
    ...
    -- for noifb mode, clean_flag should be always true
...
local function apply_br_guest(bands)
    local tblist={}

    local guestexpr = string.format("%s del dev br-guest root ", const_tc_qdisc)
    table.insert(tblist,guestexpr)

    local ratelimit= bands['DOWN'] * cfg.bands['DOWN']
    local buffer,cbuffer=get_burst(ratelimit)

    -- qdisc
    expr=string.format("%s add dev br-guest root handle 3: htb default 1000 ", const_tc_qdisc)
    table.insert(tblist,expr)

    -- class
    expr = string.format("%s add dev br-guest parent 3: classid 3:1000 htb rate %s%s quantum 3200 burst %d cburst %d",
        const_tc_class, ratelimit, UNIT, buffer, cbuffer)
    table.insert(tblist,expr)
...
function qdisc_df.apply(origin_qdisc, bands,devs, clean_flag)
    ...
    -- 更新mainframe框架
    check_if_need_update_service_mainframe(clean_flag, origin_disc, devs, cfg.bands)

    -- host_parent节点上刷新devices规则
    apply_devices_rules(devs, CLASS_HIER['child']['host'].id,CLASS_HIER['child']['guest'].id)

    -- 更新br-guest上的规则
    apply_br_guest(cfg.guest.inner)
