...
function _portConflictCheckWithProto(port, proto)
    local uci = require("luci.model.uci").cursor()
    local result = false
    uci:foreach("firewall", "redirect",
        function(s)
            if _protoHelper(s.proto) == proto or proto == 3 or _protoHelper(s.proto) == 3 then
                if _portRangeOverlap(port, s.src_dport) then
                    result = true
                end
            end
...
function setPortForward(name, ip, sport, dport, proto)
    if not XQFunction.isStrNil(ip) and _portCheck(tonumber(sport)) and _portCheck(tonumber(dport)) then
        ...
        if _portConflictCheckWithProto(sport, proto) then
            return 2
        end
        local sname = string.format("wan%srdr%s", tostring(sport), tostring(proto))
        local options = {
            ["src"]       = "wan",
            ["src_dport"] = sport,
...
function setRangePortForward(name, ip, fport, tport, proto)
    if not XQFunction.isStrNil(ip) and _portCheck(tonumber(fport)) and _portCheck(tonumber(tport)) then
        ...
        local sname = string.format("wan%srdr%s", tostring(fport), tostring(proto))
        local options = {
            ["src"]       = "wan",
            ["src_dport"] = sport,
            ["proto"]     = _protoHelper(tonumber(proto)) or "tcp",
...
function deletePortForward(port, proto)
    if _portCheck(tonumber(port)) then
        local uci = require("luci.model.uci").cursor()
        local sname = string.format("wan%srdr%s", tostring(port), tostring(proto))
...
function restart()
    local uci = require("luci.model.uci").cursor()
    local upnp_enable = uci:get("upnpd", "config", "enable_upnp") or 0
	
    os.execute("/etc/init.d/firewall restart")
    if upnp_enable then
        os.execute(XQConfigs.UPNP_DISABLE)
        os.execute(XQConfigs.UPNP_ENABLE)
    end
end
