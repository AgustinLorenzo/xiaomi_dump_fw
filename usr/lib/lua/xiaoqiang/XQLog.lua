...
function log(...)
    ...
    local debuglevel = run_cmd("uci -q get luci.debuglevel")
    if not debuglevel then
        debuglevel = '7'
    end
    if priority and tonumber(priority) and tonumber(priority) >= 0 and tonumber(priority) <= tonumber(debuglevel) then
        local util = require("luci.util")
        --posix.openlog("luci","np",LOG_USER)
        posix.openlog("luci",LOG_NDELAY,LOG_USER)
        for i = 2, arg.n do
