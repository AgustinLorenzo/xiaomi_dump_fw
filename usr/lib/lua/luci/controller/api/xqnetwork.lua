...
function index()
    ...
    entry({"api", "xqnetwork", "check_wan_link"}, call("getAutoWanLink"), (""), 218, 0x08)
    ...
    entry({"api", "xqnetwork", "set_wan_new"}, call("setWanNew"), (""), 223, 0x08)
    ...
    entry({"api", "xqnetwork", "ipv6_status"}, call("ipv6Status"), (""), 223, 0x08)
...
function setWifiAx()
    ...
    local result = {}
    local code = 0
    local ax = LuciHttp.formvalue("ax")
...
function setWifi()
    ...
    local txbf = LuciHttp.formvalue("txbf")
    local weakenable = LuciHttp.formvalue("weakenable")
    local weakthreshold = LuciHttp.formvalue("weakthreshold")
    local kickthreshold = LuciHttp.formvalue("kickthreshold")
    local ax = LuciHttp.formvalue("ax")
    ...
    if code == 0 then
        if index == 1 or index == 2 then
            local succeed = XQWifiUtil.setWifiBasicInfo(index, ssid, password, encryption, channel, txpwr, hidden, on, bandwidth, nil, txbf, weakenable, weakthreshold, kickthreshold, ax)
...
function setWifiWithoutRestart()
    ...
    if code == 0 then
        if index == 1 or index == 2 then
            local succeed = XQWifiUtil.setWifiBasicInfo(index, ssid, password, encryption, channel, txpwr, hidden, on, bandwidth, nil, txbf, nil, nil, nil, ax)
            if succeed == false then
                code = XQWifiUtil.checkWifiPasswd(password,encryption)
            end
        elseif index == 3 then
...
function setAllWifi()
    ...
    local axswitch = LuciHttp.formvalue("ax")

    local ax = nil
    if axswitch ~= nil then
        if type(axswitch) == "table" then
            ax = axswitch[1]
        else
            ax = axswitch
        end
    ...
    local succeed
    if on1 == 0 then
        succeed = XQWifiUtil.setWifiBasicInfo(1, nil, nil, nil, nil, nil, nil, on1, nil, bsd, txbf, nil, nil, nil, ax)
    else
        if ssid1 and on1 then
            code1 = XQWifiUtil.checkSSID(ssid1,31)
            if code1 == 0 then
                succeed = XQWifiUtil.setWifiBasicInfo(1, ssid1, password1, encryption1, channel1, txpwr1, hidden1, on1, bandwidth1, bsd, txbf, weakenable, weakthreshold, kickthreshold, ax)
            else
                code = code1
            end
        end
    ...
    if on2 == 0 then
        succeed = XQWifiUtil.setWifiBasicInfo(2, nil, nil, nil, nil, nil, nil, on2, nil, bsd, txbf, nil, nil, nil, ax)
    else
        if ssid2 and on2 then
            code2 = XQWifiUtil.checkSSID(ssid2,31)
            if code2 == 0 then
                succeed = XQWifiUtil.setWifiBasicInfo(2, ssid2, password2, encryption2, channel2, txpwr2, hidden2, on2, bandwidth2, bsd, txbf, weakenable, weakthreshold, kickthreshold, ax)
            else
                code = code2
            end
        end
    ...
    if txbf and ssid2 and on2 then
        succeed = XQWifiUtil.setWifiBasicInfo(2, nil, nil, nil, nil, nil, nil, nil, nil, nil, txbf, nil, nil, nil, ax)
    end
    if succeed == false then
        local error2 = {}
...
function getAutoWanLink()
    ...
    -- reset switch port 2 to eth0
    os.execute("/etc/init.d/network reload_warm 2 eth0; sleep 1")
    -- get wan port link status, read switch port 2
    local link = LuciUtil.exec("ssdk_sh port linkstatus get 2 | grep ENABLE | wc -l")
    logger.log(6, "==== getAutoWanLink() get eth0 link: " .. LuciUtil.trim(link))
    result["code"] = LuciUtil.trim(link)
    LuciHttp.write_json(result)
end
...
function setLanIp()
    ...
    local lanIp_cur = XQLanWanUtil.getLanWanIp("lan")[1]
    --logger.log(4, string.format("@@@@@@@ ip=%s ", ip))
    --logger.log(4, string.format("@@@@@@@ ip_now=%s ", lanIp_cur.ip))
    ...
    if code == 0 then
        LuciHttp.close()
        -- for D01, launch gw_update notify to all REs before activate.
...
function setWan6()
    ...
    local result = {}
    local wanType = LuciHttp.formvalue("wanType")
    local LuciNetworkIpv4 = require("luci.model.network").init()
    local wanNetworkIpv4 = LuciNetworkIpv4:get_network("wan")
    local wanTypeiIpv4 = wanNetworkIpv4:proto()
    --static
    local ip6addr = XQSecureUtil.parseCmdline(LuciHttp.formvalue("ipaddr"))
    local ip6gw = XQSecureUtil.parseCmdline(LuciHttp.formvalue("gw"))
    local ip6prefix = XQSecureUtil.parseCmdline(LuciHttp.formvalue("prefix"))
    local ip6assign = XQSecureUtil.parseCmdline(LuciHttp.formvalue("assign"))
    ...
    if XQFunction.isStrNil(wanType)
        ...
        elseif wanType == "nat" then
            if XQFunction.isStrNil(dns1) and XQFunction.isStrNil(dns2) then
                XQFunction.forkExec("sleep 2; /etc/init.d/ipv6 nat")
            elseif not XQFunction.isStrNil(dns1) and XQFunction.isStrNil(dns2) then
                XQFunction.forkExec("sleep 2; /etc/init.d/ipv6 nat " .. dns1)
            elseif XQFunction.isStrNil(dns1) and not XQFunction.isStrNil(dns2) then
                XQFunction.forkExec("sleep 2; /etc/init.d/ipv6 nat " .. dns2)
            else
                XQFunction.forkExec("sleep 2; /etc/init.d/ipv6 nat " .. dns1 .. ',' .. dns2)
            end
        elseif wanType == "static" then
            --only Ipv4 static mode can set Ipv6 static mode
            if wanTypeiIpv4 ~= "static" then
                code = 1518
            else
                if XQFunction.isStrNil(ip6assign) then
                    ip6assign = "64"
                end
                local static
                local ip6prefix = ip6prefix .. '1/' .. ip6assign
                if XQFunction.isStrNil(dns1) and XQFunction.isStrNil(dns2) then
                    static = string.format("sleep 2; /etc/init.d/ipv6 static %s %s %s %s", ip6addr, ip6gw, ip6prefix, ip6assign)
                elseif not XQFunction.isStrNil(dns1) and XQFunction.isStrNil(dns2) then
                    static = string.format("sleep 2; /etc/init.d/ipv6 static %s %s %s %s %s", ip6addr, ip6gw, ip6prefix, ip6assign, dns1)
                elseif XQFunction.isStrNil(dns1) and not XQFunction.isStrNil(dns2) then
                    static = string.format("sleep 2; /etc/init.d/ipv6 static %s %s %s %s %s", ip6addr, ip6gw, ip6prefix, ip6assign, dns2)
                else
                    static = string.format("sleep 2; /etc/init.d/ipv6 static %s %s %s %s %s,%s", ip6addr, ip6gw, ip6prefix, ip6assign, dns1, dns2)
                end
                XQFunction.forkExec(static)
            end
        elseif wanType == "off" then
            XQFunction.forkExec("sleep 2; /etc/init.d/ipv6 off")
        else
            code = 1606
        end
        if wanType ~= "off" then
            uci:set("ipv6", "settings", "ipv6_show", "1")
            uci:commit("ipv6")
            -- delete vpn6 interface while enable ipv6
...
function ipv6Status()
    local XQLanWanUtil = require("xiaoqiang.util.XQLanWanUtil")
    local wan = XQLanWanUtil.getWan6Info()
    local result = {}
    result["code"] = 0
    result["info"] = wan
    LuciHttp.write_json(result)
...
-- set wan new api, do ip confict on set_router_normal
function setWanNew()
    local XQLog = require("xiaoqiang.XQLog")
    local XQLanWanUtil = require("xiaoqiang.util.XQLanWanUtil")
    local XQFunction = require("xiaoqiang.common.XQFunction")
...
function manuallyAdd()
    ...
    local name = XQSecureUtil.hackCharsCheck(LuciHttp.formvalue("name")) or ""
    local model = tonumber(LuciHttp.formvalue("model"))
    local success = XQWifiUtil.addDevice(model, mac, name)
    if success and success == 1 then
        code = 1591
    elseif success and success == 3 then
        code = 1658
    end
    result["code"] = code
    if code ~= 0 then
...
function macBind()
    ...
    local bind = 0
    local result = {}
    local data = XQSecureUtil.xssCheck(LuciHttp.formvalue("data"))
    if data then
...
function addRedirect()
    ...
    local ip = XQSecureUtil.xssCheck(LuciHttp.formvalue("ip"))
...
function deleteRedirect()
	...
	local proto = tonumber(LuciHttp.formvalue("proto")) or 3
    if port == 0 then
        XQPortForward.deleteAllPortForward()
    else
        XQPortForward.deletePortForward(port, proto)
    end
    LuciHttp.write_json(result)
end
...
function pppoeCatch()
    ...
    local pppoecatch = XQLanWanUtil.pppoeCatch(50)
...
function appSetWifiApMode()
    ...
    result.band = LuciHttp.formvalue("band")
    result.nonce = LuciHttp.formvalue("nonce")
    result.newPwd = LuciHttp.formvalue("newPwd")
    result.oldPwd = LuciHttp.formvalue("oldPwd")
    result.nssid = LuciHttp.formvalue("nssid")
    result.nencryption = LuciHttp.formvalue("nencryption")
    result.npassword = LuciHttp.formvalue("npassword")
    result.nssid5G = LuciHttp.formvalue("nssid5G")

    if ssid then
        ap = XQAPModule.appSetWifiAPMode(ssid, password, enctype, encryption, band, channel, bandwidth, nssid, nencryption, npassword, nssid5G)
        if not ap.scan then
            result.code = 1617
        elseif ap.connected then
            result.ifname = ap.ifname
            result.ssid = ap.ssid
            result.password = ap.password
            result.enctype = ap.enctype
            result.encryption = ap.encryption
            result.conerrmsg = ap.conerrmsg
            result.oldlan =  ap.oldlan
        else
            result.code = 1616
            result["msg"] = XQErrorUtil.getErrorMessage(result.code).."("..tostring(ap.ssid)..")"
        end
    else
        result.code = 1523
    end
    ...
    local t = io.popen("cat /proc/uptime | awk '{print $1}'")
    result["uptime"] = string.trim(t:read("*all"))
    local tmpfile = assert(io.open("/tmp/luci_set_wifi_ap_mode_result", "w"))
    local results = json.encode(result)
    tmpfile:write(results)
    tmpfile:close()

    if result.code == 0 then
        -- get ip
        local dhcpcode = tonumber(os.execute("sleep 2;dhcp_apclient.sh start "..result.ifname))
        XQLog.log(6, "dhcpcode:",dhcpcode)
        if dhcpcode ~= 0 then
...
function wifiAPserviceRestart()
    ...
    local tmpfile = io.open("/tmp/luci_set_wifi_ap_mode_result", "r")
    if tmpfile == nil then
        result.code = 2
    else
        local readjson= tmpfile:read("*a")
        local results =json.decode(readjson)
        tmpfile:close()

        local t = io.popen("cat /proc/uptime | awk '{print $1}'")
        local cur_uptime = string.trim(t:read("*all"))
        if (cur_uptime - results.uptime > 300) then
            result.code = 3
        end

        if results.code and results.code==0 then
            result.code = 0
            local nonce = result.nonce
            local newPwd = result.newPwd
            local oldPwd = result.oldPwd
            if initialize == 1 and not XQFunction.isStrNil(result.ssid) then
                XQSysUtil.setRouterName(result.ssid)
                if nonce and newPwd and oldPwd then
                    result.code = _savePassword(nonce, oldPwd, newPwd)
                end
            end
            XQAPModule.setWifiAPModeConfig()
            XQSysUtil.setInited()
            XQAPModule.actionForEnableWifiAP()
        end
...
function getModeStatus()
    ...
    if tmpfile == nil then
        ...
        local results =json.decode(readjson)
        tmpfile:close()
        if results.ipaddr and results.code == 0 then
            result.code = 0
        else
            result.code = 1
        end
...
function apcli_get_signal()
    ...
    if mode == "wifiapmode" then
        ...
        if HARDWARE:match("^r3600") then
            signal = XQWifiUtil.apcli_get_real_signal(active_apcli)
            result.signal = XQWifiUtil.miwifiutil_rssi_to_signal(signal)
        end
    end
    LuciHttp.write_json(result)
end
...
function diagUsbTest()
    ...
    local date = LuciUtil.trim(LuciUtil.exec(string.format("date")))
    file:write(string.format("usb test at %s, result is:", date))
    ...
    file:write(string.format("%s\nusbtype:%s, usbspeed:%s\n", result["usbtestresult"], result["usbtype"], result["usbspeed"]))
    file:flush()
    file:close()
...
function diagHddStatus()
    ...
    local date = LuciUtil.trim(LuciUtil.exec(string.format("date")))
    file:write(string.format("disk smart info test at %s, result is:", date))
    local found_disk = 0
    local diskinfo = getDiagDiskInfo()
    local disklist = diskinfo.disklist
    for _, disk in ipairs(disklist) do
    ...
    if tonumber(found_disk) == 1 then
        ...
        if hddstatus:match("^FAIL") then
            result["hddstatus"] = "FAIL"
        end
    end
    file:write(string.format("%s\n", result["hddstatus"]))
    file:flush()
...
function diagDiskTest()
    ...
    local date = LuciUtil.trim(LuciUtil.exec(string.format("date")))
    file:write(string.format("disk rd test at %s, result is:", date))
    local diskinfo = getDiagDiskInfo()
    local disklist = diskinfo.disklist
    for _, disk in ipairs(disklist) do
        if disk.disktype:match("^SATA") then
    ...
    if tonumber(found_disk) == 1 then
        ...
        if disk_rd_test_result:match("^FAIL") then
            result["diskrdtestresult"] = "FAIL"
        end
    end
...
function addtofile(fileSrc, fileDst)
    local content
    local file1 = io.open(fileSrc, "r")
    if file1 then
...
function getDiagLog()
    ...
    if fs.access(diag_log_file) then
        local id = XQNetUtil.getSN()
        local param1, param2 = string.find(id, "/")
        local snleft = string.sub(id, 1, param2-1)
        local snwrht = string.sub(id, param2+1, string.len(id))
        local filename = snleft.."-"..snwrht.."--"..os.date("%Y-%m-%d--%X",os.time())..".diag.log"
        LuciUtil.exec("cp "..diag_log_file.." "..backuppath..filename)
        LuciUtil.exec("rm "..diag_log_file)
...
function setWifiWeakInfo()
    ...
    local succeed = XQWifiUtil.setWifiWeakInfo(index, weakenable, weakthreshold, kickthreshold)
    if succeed == false then
        code = 1502
    end
...
function miscanSwitch()
    ...
    local on = tonumber(LuciHttp.formvalue("on")) == 1 and true or false
    local switch = XQWifiUtil.miscanSwitch(on)
    if not switch then
        result.code = 1606
    end
    if result.code ~= 0 then
