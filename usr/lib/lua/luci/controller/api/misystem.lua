...
function index()
    ...
    entry({"api", "misystem", "get_config_result"},     call("getConfigResult"), (""), 104, 0x09)
    ...
    if XQFunction.isStrNil(name)
        ...
        if result.code == 0 then
            ...
            if checkssid == 0 then
                ...
                --elseif HARDWARE:match("^r3600") then
                    --XQWifiUtil.setWifiBasicInfo(1, ssid, password, "psk2+ccmp", nil, nil, 0)
                    --XQWifiUtil.setWifiBasicInfo(2, ssid.."_5G", password, "psk2+ccmp", nil, nil, 0)
                else
...
function qosSwitch()
    ...
    if result.code ~= 0 then
		...
		if HARDWARE:match("^r3600") then
			local XQFunction = require("xiaoqiang.common.XQFunction")
			XQFunction.forkExec("/etc/init.d/qca-nss-ecm restart") 
		end
        local XQSync = require("xiaoqiang.util.XQSynchrodata")
		XQSync.syncQosInfo()
    end
...
function getunConfigIotDev()
	...
	local data = {}
    local uci = require("luci.model.uci").cursor()
    local miscan = uci:get("miscan", "config", "enabled") or ""
    local wifi24GUp
    local assoclist = {}

    wifi24GOn = (tonumber(XQWifiUtil.getWifiBasicInfo(1).on) == 0)

    if miscan == "1" and wifi24GOn then
    scancmd  = "scan 1"
    ...
    if scanresult then
        for _, line in ipairs(scanresult) do
           ...
           --     if not bssid or not ssid or not model or not router_ssid or not router_bssid then
                    ssid, bssid, model, router_ssid, router_bssid = line:match("ssid:(%S+) bssid:(%S+) model:(%S+) routerSSID:(%S+) routerBSSID:(%S+)")
            --    end
            if ssid ~= nil then
            table.insert(list, {
