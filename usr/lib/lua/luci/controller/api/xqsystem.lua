...
function index()
    ...
    -- for R3600 bind info update
    entry({"api", "xqsystem", "router_bind_ok"}, call("routerBindOk"), (""), 107, 0x09)
    -- for Web only
    ...
    entry({"api", "xqsystem", "set_privacy_new"}, call("setPrivacy_new"), (""), 185, 0x09)
    -- Disk
...
function routerBindOk()
    ...
    LuciUtil.exec("ubus call trafficd bind")
...
function upgradeRom()
    ...
    if code == 0 then
        ...
        if url and filesize ~= "" and hash ~= "" then
            XQFunction.forkExec(string.format("/usr/sbin/crontab_rom.sh '%s' '%s' '%s'", XQSecureUtil.parseCmdline(url), XQSecureUtil.parseCmdline(hash), XQSecureUtil.parseCmdline(filesize)))
        else
...
function uploadLogFile()
    ...
    if HARDWARE:match("^r3600") and mode:match("^whc_cap") then
        local key = XQNetUtil.generateLogKeyV2()
        XQLog.log(6,"R3600 CAP call RE upload log, CAP key:" ..key)
...
function _checkConnection(mac, try)
    ...
    if XQWifiUtil.isDeviceWifiConnect(cmac, 1) or XQWifiUtil.isDeviceWifiConnect(cmac, 2) then
        ...
        if try > 0 then
            os.execute("sleep 3")
            local ccmac = _checkConnection(cmac, try - 1)
            if not XQFunction.isStrNil(ccmac) then
                return ccmac
            end
        end
    end
...
function chkNewStaOnRe()
    local fs = require("nixio.fs")
    local restaFile = "/tmp/new_sta_onre"
    if fs.access(restaFile) then
        local content
        local file = io.open(restaFile, "r")
        if file then
            content = file:read("*a")
            file:close()
            if content ~= nil then
...
function setVpn()
    ...
    local code = 0
    local result = {}
    local server = XQSecureUtil.xssCheck(LuciHttp.formvalue("server"))
    local username = LuciHttp.formvalue("username")
    local password = LuciHttp.formvalue("password")
...
function setCountryCode()
    ...
    local countryCodeBK = XQCountryCode.getCurrentCountryCode()
    local countryCode = LuciHttp.formvalue("country")
    if not XQCountryCode.setCurrentCountryCode(countryCode) then
        result["code"] = 1620
    else
        local cmd = "matool --method server_host_update"
        if tonumber(os.execute(cmd)) ~= 0 then
            result["code"] = 1606
            XQCountryCode.setCurrentCountryCode(countryCodeBK)
        end
    end
    if result.code ~= 0 then
        result["msg"] = XQErrorUtil.getErrorMessage(result.code)
